...
# EAR_CONSEC_FRAMES    = 20     # frames below threshold → drowsy
...
#     """Play alert only if not already playing (persistent until condition clears)."""
#     now   = time.time()
#     delay = {"drowsy": 2.5, "yawn": 3.0, "distract": 2.0, "nod": 1.8}
#     freq  = {"drowsy": 880, "yawn": 660, "distract": 550, "nod": 990}
#     if now - _last_beep.get(kind, 0) > delay.get(kind, 2.5):
#         beep(freq.get(kind, 880))
#         _last_beep[kind] = now
...
#                         if state == "ALERT":
#                             state = "DISTRACTED!"
#                         # alarm keeps ringing until they look back
#                         alert_sound("distract")
#                         ss.alert_counts["distract"] += 1
#                 else:
#                     distract_start = None
#                     looking_away   = False  # clear only when centred again
...
#                 <span class="nav-link">Safety Tips</span>
